...
async function createUserInKC(kc, userData, IDP) {
  ...
  switch (IDP) {
    ...
    case 'bceid': {
      // TODO
      console.log("TBI");
      const baseBceidGuid = userData.attributes.bceid_userid[0];

      const details = (await fetchBceidUser({ accountType: 'Business', matchKey: baseBceidGuid, TARGET_ENV })) || (await fetchBceidUser({ accountType: 'Individual', matchKey: baseBceidGuid, TARGET_ENV }));
  ...
  if (configuration.attributes.bceid_username) {
    ...
    console.log("X1:", accuserid)
    console.log("X2:", accusername)
  } else {
    // IDIR
...
const fetchBceidUser = async ({ accountType = 'Business', matchKey = '', env = 'dev' }) => {
  ...
  const filedata = fs.readFileSync('bceiddata.csv', { encoding: 'utf8' });

  const data = csvjson.toSchemaObject(filedata);
  console.log("F:", data);

  for (item of data) {
    console.log("item:", item);
    if (item.bceid_user_guid === matchKey) {
